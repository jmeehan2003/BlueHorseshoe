...
function getFunds(res, mysql, context, complete){
        mysql.pool.query("SELECT mutual_funds.id, name, style, DATE_FORMAT(inception_date, '%m/%d/%Y') AS date, CONCAT('$', FORMAT(amount_invested, 0)) AS amount_invested, category FROM mutual_funds LEFT JOIN fund_style ON mutual_funds.style = fund_style.id", function(error, results, fields){
            if(error){
                res.write(JSON.stringify(error));
                res.end();
            }
